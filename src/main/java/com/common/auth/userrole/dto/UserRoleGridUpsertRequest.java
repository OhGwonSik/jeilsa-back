...
public class UserRoleGridUpsertRequest {
    
    @Schema(description = "처리할 아이템 목록", required = true)
    @NotEmpty(message = "처리할 아이템이 최소 1개 이상 필요합니다")
    @Valid
    private List<UserRoleUpsertItem> items;
    
    @Schema(description = "일괄 처리 옵션")
    @Valid
    private OperationOptions options;
    
    public UserRoleGridUpsertRequest() {
        this.options = new OperationOptions();
    }
    
    public List<UserRoleUpsertItem> getItems() {
        return items;
    }
    
    public void setItems(List<UserRoleUpsertItem> items) {
        this.items = items;
    }
